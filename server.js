...
app.post('/login', async (req, res) => {
    ...
    console.log(`[DEBUG] Login request for role: ${role}, username: ${username}`);
    try {
        ...
        const user = userList.find(u => u.username && u.username.toLowerCase() === username.toLowerCase() && u.password === password);
        if (!user) {
            throw new Error('Invalid credentials');
        }
        console.log(`[DEBUG] Login successful for ${role} ${username}`);
        res.json({ success: true, user: { id: user.id, name: user.name, role } });
    } catch (err) {
